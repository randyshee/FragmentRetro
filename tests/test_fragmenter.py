...
def test_brics_fragmenter_visualization():
    ...
    assert (
        fragmenter.fragment_graph.number_of_nodes() > 0
    ), "Fragment graph should have nodes"
    assert (
        fragmenter.fragment_graph.number_of_edges() >= 0
    ), "Fragment graph should have edges or be empty"
