...
def test_get_length_n_combinations_from_last_stage(fragmenter_class, case_number, smiles):
    fragmenter = fragmenter_class(smiles)
    num_fragments = fragmenter.num_fragments
    last_combinations = list(fragmenter.get_length_n_combinations(1))
    for length_n in range(2, num_fragments + 1):
        length_n_combinations = fragmenter.get_length_n_combinations(length_n)
        alt_length_n_combinations = fragmenter.get_length_n_combinations_from_last_stage(last_combinations)
        last_combinations = list(length_n_combinations)
        assert (
            length_n_combinations == alt_length_n_combinations
        ), f"Case {case_number} Length {length_n}: The combinations generated by the two methods should be identical."
